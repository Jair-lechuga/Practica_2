...
int main(void)
{
	...
	while(1)
	{
		...
		{
				if (!Empty(&cajera[n]) && tiempo%atencion[n] == 0)
				{
					Sleep(TIEMPO_BASE);
					e = Dequeue(&cajera[n]);
					printf("\n\n\nAtendi a: %d en caja %d", e.n,n);
				}
		}
		...
		if(goal>=100)
		{
			for(int goal1=0;goal1<NumCajas;goal1++)
			{
				if (Empty(&cajera[goal1]))
				{
					printf("\n\n\n Ya no hay clientes formados, la tienda ha cerrado");
					exit(1);
				}
			}
		}
